#!/usr/bin/env python3
"""Build an independent, geometry-only manual audit set for R2 transitions.
...
the resulting rows are intended for operator review and validator comparison.
"""
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--sweeps",default="data/sweeps_geo05r2"); ap.add_argument("--surfaces",default="data/surfaces_v3"); ap.add_argument("--output",default="data/manual_boundary_audit.json"); ap.add_argument("--min-frames",type=int,default=60)
    ...
    for p in sorted(Path(a.sweeps).glob("**/trajectory.json")):
        ...
        for frame in frames:
            ...
            candidates.append({"frame_id":frame.get("frame_id"),"sequence_id":frame.get("sequence_id"),"facade_id":sid,"direction":frame.get("commanded_action",{}).get("direction"),"distance_m":frame.get("commanded_action",{}).get("distance_m"),"step_index":frame.get("commanded_action",{}).get("step_index"),"rgb_path":str(p.parent / (f"{frame.get('commanded_action',{}).get('direction','left').lower()}_{int(frame.get('commanded_action',{}).get('step_index',0)):04d}_rgb.png")),"manual_state":independent,"manual_boundary_pixel":line,"review_method":"independent_geometry_review_pending_operator_visual_confirmation"})
    ...
    for seq, rs in by.items():
        rs.sort(key=lambda x:x.get("step_index",0)); states=[x["manual_state"] for x in rs]
    ...
    result={"schema":"geo05r2.manual_boundary_audit.v1","independent_of":"boundary_sweep.labels","manual_review_required":True,"frames":chosen,"count":len(chosen)}
